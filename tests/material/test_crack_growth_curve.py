...
def test_pure_paris_curve():
    ...
    assert paris_pure.threshold_growth_rate == paris_pure.get_growth_rate(
        paris_pure.threshold
    )
...
def test_paris_curve_initialization():
    ...
    critical = 2000
    pc = ParisCurve(
        slope=slope, intercept=intercept, threshold=threshold, critical=critical
    )
    ...
    assert np.allclose(
        pc.get_sif(knee_gr),
        _calc_sif(knee_gr, pc.slope, pc.intercept, pc.threshold, pc.critical),
        rtol=1e-2,
    )
...
def test_paris_curve_from_knee_points():
    ...
    pc = ParisCurve.from_knee_points(
        knee_sif=knee_sif, knee_growth_rate=knee_growth_rate
    )

    assert np.allclose(pc.get_knee_sif(), knee_sif[1:-1], rtol=1e-2)
    assert np.allclose(
        pc.get_knee_growth_rate(), knee_growth_rate[1:-1], rtol=1e-2
    )
...
def test_pure_walker_curve():
    walker_pure = WalkerCurve(
        slope=3.1, intercept=1.2e-14, load_ratio=0.1, walker_exponent=0.5
    )
    ...
    assert walker_pure.threshold_growth_rate == walker_pure.get_growth_rate(
        walker_pure.threshold
    )
...
def test_paris_curve_accepts_walker_parameters():
    slope = np.array([3.1])
    intercept = np.array([1.2e-14])
    ...
    assert curve.load_ratio == 0
    assert curve.walker_exponent == 0
    np.testing.assert_allclose(curve.walker_intercept, intercept)


def test_paris_curve_plot_filters_knees_above_critical():
    curve = ParisCurve(
        slope=[3.0, 4.0, 5.0],
        intercept=[1e-12, 1e-15, 1e-20],
        threshold=10.0,
        critical=50.0,
    )
...
def test_walker_curve_from_knee_points():
    knee_sif = [20, 100, 500, 1000]
    knee_growth_rate = [1e-10, 1e-7, 1e-6, 1e-4]
    wc = WalkerCurve.from_knee_points(
        knee_sif=knee_sif, knee_growth_rate=knee_growth_rate
    )

    assert np.allclose(wc.get_knee_sif(), knee_sif[1:-1], rtol=1e-2)
    assert np.allclose(
        wc.get_knee_growth_rate(), knee_growth_rate[1:-1], rtol=1e-2
    )
...
def test_walker_curve_change_exponent():
    ...
    assert np.allclose(
        wc.get_knee_growth_rate(), pc.get_knee_growth_rate(), rtol=1e-2
    )
